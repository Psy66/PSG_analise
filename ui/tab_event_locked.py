...
# Увеличенный размер страницы для ускорения загрузки (10 000 записей на запрос)
EVENT_PAGE_SIZE = 10000
...
class EventLockedTab(BaseTab):
    ...
    def _create_widgets(self):
        # Основной контейнер с прокруткой (для левой панели)
        ...
        # Горизонтальный разделитель с равными весами
        ...
        ttk.Label(cache_frame, text=f"Страница загрузки: {EVENT_PAGE_SIZE} записей", foreground="gray").pack(anchor=tk.W)
    ...
    def _run_analysis_thread(self):
        try:
            ...
            # Общий временной массив с шагом 0.5 с
            ...
            for (pid, sev), group in df.groupby(['patient_id', 'severity']):
                ...
                if len(group) < 2:
                    continue
                # Интерполяция с ограничением экстраполяции до min/max данных
                f = interp1d(group['time_from_offset'], group['gamma_power_norm_pct'],
                             kind='linear', fill_value='extrapolate')
                y_interp = f(common_time)
                # Ограничиваем экстраполированные значения разумным диапазоном (например, [-200, 500])
    ...
    def generate_report(self):
        ...
        if self.bootstrap_results:
            boot_html += "<table border='1'><table><th>Сравнение</th><th>Метрика</th><th>Разница</th><th>95% CI</th><th>p-value</th><th>Значимо</th></table>"
            for comp, metrics in self.bootstrap_results.items():
                for metric, res in metrics.items():
